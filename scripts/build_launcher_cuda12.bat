@echo off
rem Build PhoenixAgent-V0.8.6b-CUDA12.exe — the encapsulated launcher against
rem the CUDA 12.8 toolkit. Toolkit-switch guard included (cargo does not
rem fingerprint WHICH nvcc built candle-kernels). Output lands in
rem N:\Phoenix Agent\ALPHA\Launchers\PhoenixAgent-V0.8.6b-CUDA12.exe
...
set LIB=C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.8\lib\x64;%LIB%
...
copy /Y "N:\Phoenix Agent\phoenix-agent\target\release\phoenix-agent.exe" "N:\Phoenix Agent\ALPHA\Launchers\PhoenixAgent-V0.8.6b-CUDA12.exe"
echo LAUNCHER READY: N:\Phoenix Agent\ALPHA\Launchers\PhoenixAgent-V0.8.6b-CUDA12.exe
