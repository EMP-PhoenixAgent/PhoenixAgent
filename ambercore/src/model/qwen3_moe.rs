//! Qwen3-MoE architecture (Qwen3-30B-A3B, Qwen3-235B-A22B, ...).
//!
//! This module contains a **ported copy** of candle's
//! `quantized_qwen3_moe::GGUFQWenMoE` (candle 0.11, MIT OR Apache-2.0),
//! modified in one way: candle's version keeps its per-layer
//! `ConcatKvCache`s private with **no way to reset them**, and
//! `ConcatKvCache::append` never self-resets — so reusing one built model
//! across sessions (exactly what AmberCore's replica pool does) would leak
//! the previous sequence's K/V and produce garbage. The port adds
//! [`Qwen3MoeInner::clear_kv_cache`], called by the pipeline between
//! generations (same fix AmberCore already needed for dense qwen3).
//!
//! Everything else is candle's upstream code, imports and all — the fused-MoE
//! expert path (`ffn_gate_exps` 3-D tensors), per-head q/k norms, and the
//! arch-generic metadata prefix (reads `{general.architecture}.*`, so it is
//! not qwen3-specific in principle).

// ── Ported from candle-transformers 0.11.0 src/models/quantized_qwen3_moe.rs
//    (https://github.com/huggingface/candle) — MIT OR Apache-2.0.
//    Local modifications: pub(crate) visibility, `clear_kv_cache`, docs. ──
...
impl MoeOrMlp {
    fn forward(&self, xs: &Tensor, is_prefill: bool) -> candle_core::Result<Tensor> {
        match self {
...
/// The ported candle GGUF qwen3-MoE model (see the module docs for why a
/// local copy exists: the added cache-clear).
pub struct Qwen3MoeInner {
...
impl Qwen3MoeInner {
    ...
    /// **AmberCore addition** (not in candle 0.11): reset every layer's
    /// `ConcatKvCache` so a pooled replica can be reused across sessions.
    ...
    pub fn forward(&mut self, x: &Tensor, offset: usize) -> candle_core::Result<Tensor> {
...
// ── AmberCore adapter (the DynModel the registry builds) ─────────────────────

/// A constructed Qwen3-MoE model.
pub struct Qwen3MoeModel {
    arch: String,
    inner: Qwen3MoeInner,
}

impl DynModel for Qwen3MoeModel {
    fn arch(&self) -> &str {
        &self.arch
    }
    ...
    fn forward(&mut self, input: &Tensor, index_pos: usize) -> Result<Tensor> {
        let logits = self
            ...
            .map_err(|e| Error::Model(format!("qwen3moe forward: {e}")))?;
        Ok(logits)
    }

    fn clear_kv_cache(&mut self) {
        // MANDATORY: the ConcatKvCache appends unconditionally (no pos-0
        // self-reset), exactly like dense qwen3. See the module docs.
        self.inner.clear_kv_cache();
    }
}

/// Registry entry point: construct a Qwen3-MoE model from a loaded GGUF.
///
/// Uses F32 throughout the stack (correctness-first; the dtype parameter is
/// kept for a future CUDA f16 fast path).
pub fn build(loaded: &mut LoadedModel, device: &Device) -> Result<Box<dyn DynModel>> {
    ...
    let inner = Qwen3MoeInner::from_gguf(content, &mut loaded.file, device, DType::F32)
        .map_err(|e| Error::Model(format!("qwen3moe from_gguf: {e}")))?;
    Ok(Box::new(Qwen3MoeModel { arch, inner }))
}
